...
public class AlertQueue(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<CreateAlertHandler> logger) : BackgroundService
{
                ...
                try
                {
                    ...
                    consumer.ReceivedAsync += async (_, ea) =>
                    {
                        try
                        {
                            ...
                            var request = JsonSerializer.Deserialize<CreateAlertRequest>(message);
                            if (request is null)
                            {
                            ...
                            if (response.IsSuccess)
                                logger.LogInformation("Alert processed successfully for AlertId: {AlertId}", response.Value);
                            else
                                logger.LogError("Alert processing failed: {Error}", response.Exception?.Message);
                        }
                        catch (Exception ex)
                        {
